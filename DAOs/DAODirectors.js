...
class DAODirectors {
    ...
    newDirector(director, callback) {
        this.pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(
                    'INSERT INTO DIRECTORS (fullname, name, surname, email, phone, nationality, birth_city, home_city, DNI, birthdate, age, esp_bio, eng_bio, address, street, city, postalcode, country, web, notes, modif) VALUES ?',
                    [director],
                    (err) => {
                        if (err) {
                            callback(err)
    ...
    updateDirector(director, id, callback) {
        this.pool.getConnection((err, connection) => {
            ...
            connection.query(
                'UPDATE DIRECTORS SET fullname = ?,name = ?,surname = ?,email = ?,phone = ?, nationality = ?, birth_city = ?, home_city = ?, DNI = ?, birthdate = ?, age = ?, esp_bio = ?, eng_bio = ?, address = ?, street = ?, city = ?, postalcode = ?, country = ?, web = ?, notes = ?, modif = ? WHERE id = ?',
                [
                    director.fullname,
                    director.name,
                    director.surname,
    ...
    getDirector(id, callback) {
        this.pool.getConnection((err, connection) => {
            ...
            connection.query('SELECT * FROM DIRECTORS WHERE id = ?', [id], (err, director) => {
                connection.release()
                if (err) {
                    callback(err, undefined)
                } else {
    ...
    getDirectors(callback) {
        this.pool.getConnection((err, connection) => {
            ...
            connection.query('SELECT * FROM DIRECTORS', (err, directors) => {
                connection.release()
                if (err) {
                    callback(err, null)
                } else {
