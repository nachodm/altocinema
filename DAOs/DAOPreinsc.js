...
class DAOPreinsc {
    ...
    getPreinsc(callback) {
        this.pool.getConnection((err, connection) => {
            ...
            connection.query(
                'SELECT B.name, C.title FROM PREINSCR AS A JOIN FESTIVALS AS B ON A.festival_id = B.id JOIN FILMS AS C ON A.film_id = C.id',
                (err, preinscr) => {
                    connection.release()
                    if (err) {
                        callback(err, null)
    ...
    handleMonthPreinscriptions(callback) {
        this.pool.getConnection((err, connection) => {
            ...
            connection.query(
                'SELECT * FROM ( SELECT festcat.id as festid, COUNT(festcat.id) AS SUMA, filcat.id, fest.name, fil.id AS filmId ' +
                ...
                (err, preinscriptions) => {
                    if (err) {
                        ...
                        if (preinscriptions.length > 0) {
                            ...
                            connection.query(
                                'INSERT INTO PREINSCR(festival_id, film_id) VALUES ?',
                                [ids],
                                (err) => {
                                    if (err) {
                                        connection.release()
