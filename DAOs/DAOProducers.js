...
class DAOProducers {
    ...
    newProducer(producer, callback) {
        this.pool.getConnection((err, connection) => {
            if(err){
                ...
                connection.query("INSERT INTO PRODUCERS (fullname, name, surname, email, phone, nationality, home_city, birthdate, age, notes, modif) VALUES ?",
                [producer],
                (err) => {
                    if (err) { callback(err);}
                    else {
    ...
    updateProducer(producer, id, callback){
        this.pool.getConnection((err, connection) => {
            ...
            connection.query("UPDATE PRODUCERS SET fullname = ?,name = ?,surname = ?,email = ?,phone = ?, nationality = ?, home_city = ?, birthdate = ?, age = ?, notes = ?, modif = ? WHERE id = ?",
            [producer.fullname, producer.name, producer.surname, producer.email, producer.phone, producer.nationality, producer.home_city, producer.birthdate, producer.age, producer.notes, producer.modif, id],
            (err) => {
                connection.release();
    ...
    getProducer(id, callback) {
        this.pool.getConnection((err, connection) => {
            ...
            connection.query("SELECT * FROM PRODUCERS WHERE id = ?",
            [id],
            (err, producer) => {
                connection.release();
